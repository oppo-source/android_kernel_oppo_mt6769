...
static void tianma_init_set_cabc(struct tianma *ctx, int cabc_mode)
{
	...
	if (cabc_mode == 0) {
		...
		tianma_dcs_write_seq_static(ctx, 0x55, 0x03);
	} else {
		tianma_dcs_write_seq_static(ctx, 0x55, 0x00);
	}
}
...
static struct mtk_panel_params ext_params = {//60hz
	...
	.dyn = {
		...
		.vsa = MODE_0_HOPPING_VSA,
		.vfp = MODE_0_HOPPING_VFP,
		.hbp = MODE_0_HOPPING_HBP,
		.vfp_lp_dyn = MODE_0_HOPPING_VFP_LP_DYN,
	},
#ifdef CONFIG_MTK_ROUND_CORNER_SUPPORT
	.round_corner_en = 1,
...
static struct mtk_panel_params ext_params_mode_1 = {//90hz
	...
	.dyn = {
		...
		.pll_clk = HOPPING_MIPI_CLK,
		.data_rate = HOPPING_DATA_RATE,
		.vsa = MODE_1_HOPPING_VSA,
		.vfp = MODE_1_HOPPING_VFP,
		.hbp = MODE_1_HOPPING_HBP,
		.vfp_lp_dyn = MODE_1_HOPPING_VFP_LP_DYN,
	},
#ifdef CONFIG_MTK_ROUND_CORNER_SUPPORT
	.round_corner_en = 1,
	.corner_pattern_height = ROUND_CORNER_H_TOP,
